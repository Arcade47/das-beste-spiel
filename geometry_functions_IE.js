...
function getXY_exact(e, xscale, yscale) {
  var xscale = xscale || 1;
  var yscale = yscale || 1; // return exact position relative to canvas (for physics)
  ...
  var return_y = y_exact / yscale; // mind that canvas is transformed (translated) in css

  return_x += canv_w / 2;
  return_y += canv_h / 2; // return_x += window.innerHeight/2;
...
function initialize_grid(n_cols, n_rows) {
  ...
  for (var i = 0; i < n_cols; i++) {
    polys_grid_state.push([]); // filling with zeros
...
function pos_to_cells(pos) {
  ...
  to_be_tested.push({
    col: Math.round(pos.x / px_per_col) - 1,
    row: Math.round(pos.y / px_per_row) - 1
  });
  to_be_tested.push({
    col: Math.round(pos.x / px_per_col) - 1,
    row: Math.round(pos.y / px_per_row)
  });
  to_be_tested.push({
    col: Math.round(pos.x / px_per_col),
    row: Math.round(pos.y / px_per_row) - 1
  });
  to_be_tested.push({
    col: Math.round(pos.x / px_per_col),
...
function pos_to_cells_random(pos) {
  ...
  var halfy = px_per_row / 2; // special case: exactly in middle, then not two coords
...
function get_vertical_cells_string(line) {
  ...
  var halfy = px_per_row / 2; //TODO issue: sometimes x coord not correct

  var all_cells = []; // get the x coordinate (always the same)

  var colcell = pos_to_cell(line[0]).col;
  var first_cells = pos_to_cells_random(line[0]); // take care that random function does not mess up x

  first_cells_correct_col = keep_col_val_equal(first_cells, colcell); // append first cells

  all_cells = multi_append(all_cells, first_cells_correct_col); // snap to between grid

  var first_step = 1.5 * px_per_row - line[0].y % px_per_row;
  var ycoord = line[0].y + first_step;
  var snd_cells = pos_to_cells_random({
    x: line[0].x,
    y: ycoord
  }); // take care that random function does not mess up x

  snd_cells_correct_col = keep_col_val_equal(snd_cells, colcell); // append

  all_cells = append_cells_avoid_doubles(all_cells, snd_cells_correct_col); // loop from p1 snapped between grid to p2 until > line

  for (var posy = ycoord; posy < line[1].y; posy += px_per_row) {
    var third_cells = [pos_to_cell({
      x: line[0].x,
      y: posy
    })]; // take care that random function does not mess up x
  ...
  var last_cells = pos_to_cells_random({
    x: line[0].x,
    y: line[1].y
  }); // take care that random function does not mess up x

  last_cells_correct_col = keep_col_val_equal(last_cells, colcell);
  all_cells = append_cells_avoid_doubles(all_cells, last_cells_correct_col); // return the finished list
...
function pos_to_cell(pos) {
  ...
  var px_per_row = canv_h / n_rows;
  var halfx = px_per_col / 2;
  var halfy = px_per_row / 2; // x

  var colcell = (pos.x - halfx) / px_per_col; // y
...
function line_to_grid_cells(line) {
  ...
  for (var i = aabb_inds[0].col; i <= aabb_inds[1].col; i++) {
    for (var j = aabb_inds[0].row; j <= aabb_inds[1].row; j++) {
      ...
      var upper_bound = {
        x: px_per_col * i + px_per_col,
        y: px_per_row * j + px_per_row
      };
...
function create_vertex_pos(x, y) {
  px_per_col = canv_w / n_cols;
  px_per_row = canv_h / n_rows; // index of line in grid

  var x_ind = Math.round(x / px_per_col);
  var y_ind = Math.round(y / px_per_row); // find rounded pos

  var x_ind_in_pos = px_per_col * x_ind;
  var y_ind_in_pos = px_per_row * y_ind; // make vertex object
...
function line_line_inters(line1, line2) {
  ...
  if (slope(line1) == slope(line2)) {
    ...
    if (intercept(line1) == intercept(line2)) {
      return Infinity;
    } else {
      return NaN;
    }
...
function lineseg_lineseg_inters(line1, line2) {
  ...
  aabb2 = aabb_from_coords(line2); // no intersection if aabbs do not overlap

  if (!overlap_2_dim(aabb1, aabb2)) {
    return NaN;
  } else {
    return line_line_inters(line1, line2);
  }
}
...
function bound_line_horizontal(line, lowerb, upperb) {
  ...
  p1 = {};
  p2 = {}; // special cases first: smaller as bounds

  if (ordered_l[0].x > lowerb) {
    p1.x = ordered_l[0].x;
    p1.y = ordered_l[0].y;
  } else {
    lowerb_line = [{
...
function aabb_from_coords(coords) {
  ...
  var sortx = [Math.min.apply(Math, _toConsumableArray(x_vals)), Math.max.apply(Math, _toConsumableArray(x_vals))];
  var sorty = [Math.min.apply(Math, _toConsumableArray(y_vals)), Math.max.apply(Math, _toConsumableArray(y_vals))];
  return [{
    x: sortx[0],
    y: sorty[0]
  }, {
...
function overlap_1_dim(s11, s12, s21, s22) {
  ...
  sorted_s2 = order_2_vals(s21, s22); // no overlap if any larger value is smaller as the other's smaller value

  if (sorted_s1[1] < sorted_s2[0]) {
    return false;
  }

  if (sorted_s2[1] < sorted_s1[0]) {
    return false;
  } // otherwise: overlap
...
function overlap_2_dim(aabb1, aabb2) {
  ...
  // check in 1st dim
  overlap_1st = overlap_1_dim(aabb1[0].x, aabb1[1].x, aabb2[0].x, aabb2[1].x); // check in 2nd dim

  overlap_2nd = overlap_1_dim(aabb1[0].y, aabb1[1].y, aabb2[0].y, aabb2[1].y);

  if (!overlap_1st || !overlap_2nd) {
    return false;
  } else {
    return true;
  }
...
function line_in_cell(line, lowerb, upperb) {
  ...
  if (slope(line) == 0) {
    if (line[0].y >= lowerb.y && line[0].y <= upperb.y) {
      return true;
    } else {
      return false;
    }
  ...
  if (slope(line) == Infinity) {
    if (line[0].x >= lowerb.x && line[0].x <= upperb.x) {
      return true;
    } else {
      return false;
    }
  ...
  x_bounded_line = bound_line_horizontal(line, lowerb.x, upperb.x); // check overlap in other dimension
...
function aabb_from_cell_inds(cell_inds) {
  ...
  s_col = Math.min.apply(Math, _toConsumableArray(col_inds));
  l_col = Math.max.apply(Math, _toConsumableArray(col_inds));
  s_row = Math.min.apply(Math, _toConsumableArray(row_inds));
  l_row = Math.max.apply(Math, _toConsumableArray(row_inds));
  return [{
    col: s_col,
    row: s_row
  }, {
...
function get_inds_around_vertex(pos) {
  ...
  var ly = Math.round(pos.y / px_per_row); // catch case when outside canvas

  if (sx < 0 || lx >= n_cols || sy < 0 || ly >= n_rows) {
    return output;
  }
...
function aabb_cell_within_canvas(aabb) {
  ...
  if (aabb[0].col < 0 || aabb[0].row < 0) {
    return false;
  }

  if (aabb[1].col >= n_cols || aabb[1].row >= n_rows) {
    return false;
  } // no problems:
...
function first_cell_from_dir(cell_string, dir) {// dir: 'f' ||'b' (forward, backward)
}

function get_dirs_from_celldiff(celldiff) {
  var dirs = []; // positive: leftwards or upwards
...
function inverse_dirs(dirs) {
  var inv_dirs = []; // handy function for collision detection
...
function cells_overlap(cells_o, cells_s) {
  // similar cells
  var sim_cells = []; // compare each cell with any other
...
function get_cells_from_dir(cells, dir, grid, ind) {
  ...
  // output variable:
  var cells_from_dir = []; // problem: cells as input not ordered: needs complete grid w/ AABB
  // derive aabb

  aabb = aabb_from_cell_inds(cells); // case distinctions: l/r --> outer loop over rows, inner over cols (vice versa for t/b)
  ...
  for (var i = start_ol; i <= end_ol; i++) {
    var found = false; // distinction between increments and decrements
...
function gradual_diff_list(diff_cells) {
  ...
  var output = []; // append the starting difference (making a copy of object is necessary...)
  ...
  while (remain_diff.row != 0 || remain_diff.col != 0) {
    if (current_turn == 'col') {
      ...
      if (remain_diff.col != 0) {
        ...
        current_turn = 'row'; // if no diff: other dimension must have it
      } else {
      ...
      if (remain_diff.row != 0) {
        remain_diff.row += rdir;
        current_turn = 'col'; // if no diff: other dimension must have it
      } else {
        remain_diff.col += cdir;
      }
    } // append to list
...
function cell_to_coord(cell) {
  ...
  var px_per_row = canv_h / n_rows;
  return {
    x: cell.col * px_per_col,
    y: cell.row * px_per_row
  };
...
function fill_poly(coords, poly_grid_state) {
  ...
  var all_cells = []; // loop through every cell in aabb
  ...
  for (var i = aabb[0].col; i <= aabb[1].col; i++) {
    ...
    for (var j = aabb[0].row; j <= aabb[1].row; j++) {
      ...
      if (poly_grid_state[i][j] == -1) {
        col_active = true; // append the boundary cells
      ...
      if (col_active && poly_grid_state[i][j] == 0) {
        // first empty cell detected --> collect
        collected_string.push({
          col: i,
          row: j
        });
      ...
      if (col_active && poly_grid_state[i][j] == -1 && collected_string.length > 0) {
        col_active = false; // append the collected cells to all cells
...
function fill_coords(coords) {
  ...
  var aabb = aabb_cells_from_line(aabb_coords); // get all line cells

  var line_cells = lines_from_coords(coords, true); // output list

  var all_cells = []; // convert coords to cells!
  // first get list of lines

  var lineslist = lines_from_coords(coords); // loop over each line and push to cells variable
  ...
  for (var i = aabb[0].col; i <= aabb[1].col; i++) {
    ...
    for (var j = aabb[0].row; j <= aabb[1].row; j++) {
      ...
      if (col_active && cell_in_cells({
        col: i,
        row: j
      }, cells) && collected_string.length > 0) {
        col_active = false; // append the collected cells to all cells
...
function fill_poly_from_coords(coords) {
  ...
  halfx = px_per_col / 2;
  px_per_row = canv_h / n_rows;
  halfy = px_per_row / 2; // get aabb of coords

  var aabb_coords = aabb_from_coords(coords); // loop over each column
  ...
  for (var posx = xstart; posx + halfx <= aabb_coords[1].x; posx += xstep) {
    ...
    var p1 = {
      x: posx,
      y: aabb_coords[0].y - halfy
    };
    var p2 = {
      x: posx,
      y: aabb_coords[1].y + halfy
    }; // create the line

    var current_line = [p1, p2]; // find the intersections with poly lines

    var all_inters = []; // list to append to

    var all_lines = lines_from_coords(coords, false); // loop through all the lines of poly

    for (var i = 0; i < all_lines.length; i++) {
      var inters = lineseg_lineseg_inters(all_lines[i], [p1, p2]); // append if there is intersection with poly line
    ...
    all_inters = sort_vertical_intersections(all_inters); // loop through all intersections

    for (var i = 0; i < all_inters.length; i++) {
      fill_line.push(all_inters[i]); // line completed:

      if (fill_line.length == 2) {
        var cells_line = get_vertical_cells_string(fill_line);
        fill_line = []; // fill into list of all_cells
  ...
  all_cells = append_cells_avoid_doubles(all_cells, last_cells); // round all cells!
  ...
  for (var i = 0; i < all_cells.length; i++) {
    // make sure to round
    var cell = {
      col: Math.round(all_cells[i].col),
      row: Math.round(all_cells[i].row)
    };
...
function get_rate(input, min, max) {
  if (input < min) {
    ...
    return input / max; // var diff = max - min;
