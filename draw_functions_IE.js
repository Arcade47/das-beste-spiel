...
var canv_h = canvas.height; // var horizontal_screen = true;
...
function draw_speech_bubble(text, pos) {
  ...
  ctx.beginPath(); // assuming edge of bubble starts 10 px to right and above pos

  for (var index = 0; index < coords.length; index++) {
    var coord = coords[index];
    ctx.lineTo.apply(ctx, _toConsumableArray(coord)); //moveTo
  }

  ctx.closePath();
  ctx.stroke();
  ctx.fill();
  draw_canvas_text_flex(text, {
    x: pos.x + 0.025 * canv_h,
    y: pos.y - 0.025 * canv_h - 0.01 * canv_h
  }, "black", 0.9 * font_size, align = "center");
}

function draw_textbox(lines, pos, len) {
  var len = len || 1;
  var w = len * (canv_w / 8);
  var h = canv_w / 35 * lines.length + canv_w / 50; // center pos
  ...
  for (var index = 0; index < lines.length; index++) {
    ...
    draw_canvas_text_flex(line, {
      x: pos.x + w / 2,
      y: pos.y + canv_w / 35 + canv_w / 100
    }, "DarkViolet", size, align = "center");
    pos.y += step;
  }
}
...
function testdraw_polys_cells(color, list_of_polys_cells) {
  for (var i = 0; i < list_of_polys_cells.length; i++) {
    for (var j = 0; j < list_of_polys_cells[i].length; j++) {
      highlight_cell({
        col: list_of_polys_cells[i][j].col,
        row: list_of_polys_cells[i][j].row
      }, color);
    }
  }
}
...
function draw_cursor(pic, pos) {
  ctx.drawImage(pic, 0, 0, 80, 80, pos.x - 10, pos.y - 10, 20, 20);
}

function draw_highscores(pair) {
  set_canvas_bg("black");
  draw_canvas_text_flex("HIGHSCORES", {
    x: canv_w / 2,
    y: canv_h / 10
  }, "white", canv_h / 15, align = "center");
  var ypos = 2 * (canv_h / 9);

  for (var index = 0; index < pair.length; index++) {
    var splitted = pair[index].split("|a|");
    draw_canvas_text_flex(String(index + 1) + ".", {
      x: canv_w / 2 - 2 * canv_w / 5,
      y: ypos
    }, "white", canv_h / 15, align = "left");
    draw_canvas_text_flex(splitted[0], {
      x: canv_w / 2 - canv_w / 3,
      y: ypos
    }, "white", canv_h / 15, align = "left");
    draw_canvas_text_flex(splitted[1] + " €", {
      x: canv_w - (canv_w / 2 - 2 * canv_w / 5),
      y: ypos
    }, "white", canv_h / 15, align = "right");
    ypos += canv_h / 9;
  }

  draw_canvas_text_flex("Click / tap to try again", {
    x: canv_w / 2,
    y: canv_h - canv_h / 10
  }, "white", canv_h / 15, align = "center");
}
...
function show_instructions1() {
  set_canvas_bg("black");
  var lines = ["In der folgenden Aufgabe sollen Sie in", "begrenzter Zeit so viel Geld wie möglich erwirtschaften.", "Sie generieren Geld, indem Sie im Büro arbeiten", "und genügend Mitarbeiter produktiv sind.", "Sie können Mitarbeiter produktiv halten,", "indem Sie auf die entsprechenden Türen klicken.", "Beachten Sie aber, dass Sie dann für eine", "gewisse Zeit nicht im Büro sind.", "", "- Clicken/Tippen für nächste Seite -"];
  var ypos = canv_h / 10;
  var step = canv_h / 12;
  var size = canv_w / 35;
...
function show_instructions2() {
  ...
  set_canvas_bg("black");
  var lines = ["Sobald genügend Geld erwirtschaftet ist", "und wenn alle Mitarbeiter produktiv sind,", "gibt es Gelegenheiten, das Gebäude zu erweitern", "und mehr Mitarbeiter einzustellen.", "Die Kosten pro neue Tür belaufen sich auf", String(cost_per_door) + " €. Kein Rabatt.", "Mehr produktive Mitarbeiter - mehr Geld.", "Entscheiden Sie selbst.", "", "- Clicken/Tippen, um zum Menü zurückzukehren -"];
  var ypos = canv_h / 10;
  var step = canv_h / 12;
  var size = canv_w / 35;
...
function start_screen(arrows) {
  set_canvas_bg("black");
  var lines = ["DAS BESTE(-)SPIEL", "", "Anleitung", "Spiel starten"];
  var ypos = canv_h / 5;
  var step = canv_h / 7;
  var size = canv_w / 30;
  ...
  for (var _index = 0; _index < arrows.length; _index++) {
    var a = arrows[_index];
...
function draw_arrow(rad) {
  ...
  var pos = {
    x: 0.5 * canv_w,
    y: canv_h / (6 * 2) + arrow_width / 2
  };
  var middle_coord = {
    x: pos.x + 0.5 * arrow_width,
    y: pos.y - 0.5 * arrow_width
  }; // line

  var coords = [];
  var coord2 = rad_to_coord(rad, 0.85 * arrow_width);
  coords.push({
    x: middle_coord.x + coord2.x,
    y: middle_coord.y + coord2.y
  });
  coords.push({
    x: pos.x + 0.5 * arrow_width,
    y: pos.y - 0.5 * arrow_width
  ...
  coords_poly.push({
    x: middle_coord.x + coord_poly1.x,
    y: middle_coord.y + coord_poly1.y
  });
  coords_poly.push({
    x: middle_coord.x + coord_poly2.x,
    y: middle_coord.y + coord_poly2.y
  });
  coords_poly.push({
    x: middle_coord.x + coord_poly3.x,
    y: middle_coord.y + coord_poly3.y
  }); // color is determined by rate

  var rate = get_rate_for_arrow(rad); // draw_debug_text(round_digits(rate, 2), {x: pos.x + 200, y: pos.y});
  ...
  var color = get_color_label(R, G, 0, 1); // draw with the specs
